...
def mirrorMesh(clean_mesh = '', target_to_mirror = ''):
    """
    Mirror a Shape on a Mesh without changing vertex Id
    """
    def findWrap( _tranformToCheck):
        """
        Find wrap node from a string
        @param _tranformToCheck: Needs to be a String!!
        """
        result = []
        if not (pm.objExists(_tranformToCheck)):
        ...
        for elem in validList:
            if pm.nodeType(elem) == 'wrap':
                result.append(elem)
        return result

    mirrored_target = mc.duplicate(clean_mesh, n='{}_MIRRORED_TARGET'.format(clean_mesh))
    wrap_target = mc.duplicate(clean_mesh, n='{}_WRAP'.format(clean_mesh))

    mc.setAttr('{}.sx'.format(wrap_target[0]), -1)
    bls = mc.blendShape(target_to_mirror, wrap_target, 
                  o='local', w=[(0, 0)], foc=False)[0]
                  
    mc.select(mirrored_target, r=True)
    mc.select(wrap_target, add=True)
    mc.CreateWrap()
    mc.blendShape(bls, e=1 , w=[(0, 1)])
        
    wrap = findWrap(mirrored_target[0])[0]
    baseMesh = mc.listConnections('{}.basePoints'.format(wrap), p=0, s=1, d=0)[0]

    mc.DeleteHistory(mirrored_target)
    mc.delete(baseMesh)
    mc.delete(wrap_target)
