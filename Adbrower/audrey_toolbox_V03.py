...
#-----------------------------------
    ...
    def addSeparator(self):
        ...
        for i in range(0, 100):
            if i == 100:
                pm.mel.error("There are more than 20 seperators. Why would you even need that many.")
    ...
    def rmvConstraint(self):
        for i, each in enumerate(pm.selected()):
            parentConstraints = (
                set([x for x in each.inputs() if type(x) == pm.nodetypes.ParentConstraint]))
            pm.delete(parentConstraints)
            scaleConstraints = (
                set([x for x in each.inputs() if type(x) == pm.nodetypes.ScaleConstraint]))
            pm.delete(scaleConstraints)
            orientConstraints = (
                set([x for x in each.inputs() if type(x) == pm.nodetypes.OrientConstraint]))
            pm.delete(orientConstraints)
            pointConstraints = (
                set([x for x in each.inputs() if type(x) == pm.nodetypes.PointConstraint]))
            pm.delete(pointConstraints)
            aimConstraints = (
                set([x for x in each.inputs() if type(x) == pm.nodetypes.AimConstraint]))
    ...
    def add_material(self, mat_name='mat_yellow'):
        ...
        if pm.objExists(mat_name):
            ...
            pm.setAttr(LambertYellow + ".color",
                       mat_dict[mat_name], type='double3')
    ...
    def allgrey(self):
        ...
        allColors = list(
            set([pm.PyNode(x).overrideColorRGB.get(x) for x in allNurbs]))
        nub = len(allColors)
