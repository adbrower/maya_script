...
from adbrower import changeColor, lprint
...
class markingMenu():
    """
    The main class, which encapsulates everything we need to build and rebuild our marking menu.
    All done in the constructor, so all we need to do in order to build/update our marking menu is
    ...
    def _buildMarkingMenu(self, menu, parent):
        ...
        mc.menuItem(p=menu, l="Blendshape - Local",  c=lambda *args: adb.blendshape(pm.selected()[-1], pm.selected()[0:-1], origin="local"))
        mc.menuItem(p=menu, l="Blendshape - World (snap to position)",  c=lambda *args: adb.blendshape(pm.selected()[-1], pm.selected()[0:-1], origin="world"))
